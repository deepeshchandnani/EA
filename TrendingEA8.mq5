...
void PlaceMarketOrder(string symbol, bool isBuy, double finalSL, double finalTP)
  {
   bool result;
   if(isBuy)
      result = trade.Buy(g_CurrentLot, symbol, 0, finalSL, finalTP, "");
   else
      result = trade.Sell(g_CurrentLot, symbol, 0, finalSL, finalTP, "");
   ...
   if(isBuy)
     {
      if(entry > freshPrice)
        {
         result = trade.BuyStop(g_CurrentLot, entry, symbol, finalSL, finalTP, timeType, expiration, "");
         orderLabel = "BUY STOP";
        }
      else
        {
         result = trade.BuyLimit(g_CurrentLot, entry, symbol, finalSL, finalTP, timeType, expiration, "");
   ...
   else
     {
      if(entry < freshPrice)
        {
         result = trade.SellStop(g_CurrentLot, entry, symbol, finalSL, finalTP, timeType, expiration, "");
         orderLabel = "SELL STOP";
        }
      else
        {
         result = trade.SellLimit(g_CurrentLot, entry, symbol, finalSL, finalTP, timeType, expiration, "");
