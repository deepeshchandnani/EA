...
//| Sequence sirf 2 tarike se khatam hoti hai: TP1 hit, ya signal-SL   |
//| (kill-switch) breach. Koi bhi aur closure (manual/unexpected) bhi  |
//| ab reentry hi maana jata hai - loop continue rehta hai.            |
//+------------------------------------------------------------------+
#property copyright ""
...
input double InpLotSize        = 0.4;         // Base lot size (martingale isi se start hota hai)
...
input int    InpPriceCheckMillis = 200;       // Price monitor kitni fast check ho (milliseconds) - jitna kam utna fast reaction
input int    InpFolderScanSeconds = 2;        // Signal folder kitni der me scan ho (seconds) - price check se alag, kam frequent chalta hai
...
int OnInit()
  {
   ...
   g_HeartbeatEveryNTicks  = (int)MathMax(1, MathRound(30000.0 / InpPriceCheckMillis)); // ~30 sec heartbeat

   EventSetMillisecondTimer(InpPriceCheckMillis);
   Print("Fast timer set: har ", InpPriceCheckMillis, "ms price monitor hoga. Folder scan har ~", InpFolderScanSeconds, " sec (", g_FolderScanEveryNTicks, " ticks).");
   Print("=====================================================");
      ...
      if(g_Sequence.active && g_Sequence.positionOpen && (ulong)positionId == g_Sequence.currentTicket)
        {
         if(dealReason == DEAL_REASON_TP)
           ...
           {
            // Unexpected/manual close (jaise kisi ne terminal me manually close kar diya, ya broker
            // stop-out, ya koi aur wajah) - ab ye bhi REENTRY jaisa treat hota hai, sequence khatam
            // NAHI hota, martingale touch nahi hoti, wapas usी entry price par wait karega
            Print("[REENTRY] Unexpected/manual close hua (deal #", dTicket, " profit=", profit, " reason=", dealReason,
                  ") - martingale untouched, sequence continue. Wapas entry (", g_Sequence.entry, ") ke liye wait kar rahe hain.");
            g_Sequence.positionOpen = false;
            g_Sequence.currentTicket = 0;
            g_Sequence.closeIntent = "NONE";
            g_Sequence.lastPrice = g_Sequence.isBuy ? SymbolInfoDouble(g_Sequence.symbol, SYMBOL_ASK)
...
void OnTimer()
  {
   ...
   // Price monitoring HAR tick pe (fast reaction ke liye) - martingale/sequence check
   CheckClosedTrades();
   CheckActiveSequence();

   // Folder scan sirf har N ticks pe (file I/O itni frequent karne ki zaroorat nahi)
