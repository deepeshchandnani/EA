...
PHONE_NUMBER = "+91XXXXXXXXXX"        # tumhara Telegram number
...
SIGNAL_FOLDER = r"C:\Path\To\MT5\MQL5\Files\Signals"
...
async def handler(event):
    ...
    # MT5 EA ab simple single-entry design use karta hai (loop/dual-leg hata diya gaya hai) -
    # backward-compatible "entry"/"sl" fields bhi bhej dete hain
    mt5_signal = dict(signal)
    mt5_signal["entry"] = signal["entry_deep"]  # single entry - purani min(BUY)/max(SELL) logic
    mt5_signal["sl"] = signal["sl_raw"]          # asli signal ka SL - ab isi ko broker SL banayenge
