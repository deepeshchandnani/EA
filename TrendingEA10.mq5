...
input int    InpDefaultSLPips  = 50;          // (Ab use nahi ho raha SL ke liye - neeche InpFixedSLDollars dekho)
input double InpFixedSLDollars = 10.0;        // SL hamesha entry se itne $ door rahega (signal ka SL IGNORE hota hai) - 1 pip = $1 maan kar
input int    InpDefaultTPPips  = 100;         // (Ab use nahi ho raha TP ke liye - neeche InpFixedTPDollars dekho)
input double InpFixedTPDollars = 5.0;         // TP hamesha entry se itne $ door rahega (signal ka TP IGNORE hota hai) - 1 pip = $1 maan kar
...
void ExecuteTrade(string symbol, string direction, double entry, double sl, double tp)
  {
   ...
   // SL ab HAMESHA fixed $ distance se calculate hoti hai - signal me jo SL diya ho use IGNORE karte hain
   // 1 pip = $1 movement maan kar: BUY ke liye entry se neeche, SELL ke liye entry se upar
   double finalSL = isBuy ? refPrice - InpFixedSLDollars : refPrice + InpFixedSLDollars;

   // TP bhi ab HAMESHA fixed $ distance se calculate hoti hai - signal me jo TP diya ho use IGNORE karte hain
   double finalTP = isBuy ? refPrice + InpFixedTPDollars : refPrice - InpFixedTPDollars;
   ...
   Print("[TRADE PLAN] ", direction, " ", symbol, " | ask=", ask, " bid=", bid,
         " | finalSL=", finalSL, " (fixed $", InpFixedSLDollars, " gap, signal SL=", sl, " ignored)",
         " | finalTP=", finalTP, " (fixed $", InpFixedTPDollars, " gap, signal TP=", tp, " ignored)",
         " | entry(from signal)=", entry);
...
void PlaceMarketOrder(string symbol, bool isBuy, double finalSL, double finalTP)
  {
   bool result;
   if(isBuy)
      result = trade.Buy(g_CurrentLot, symbol, 0, finalSL, finalTP, "EMA movement detected!");
   else
      result = trade.Sell(g_CurrentLot, symbol, 0, finalSL, finalTP, "EMA movement detected!");
   ...
   if(isBuy)
     {
      if(entry > freshPrice)
        {
         result = trade.BuyStop(g_CurrentLot, entry, symbol, finalSL, finalTP, timeType, expiration, "EMA movement detected!");
         orderLabel = "BUY STOP";
        }
      else
        {
         result = trade.BuyLimit(g_CurrentLot, entry, symbol, finalSL, finalTP, timeType, expiration, "EMA movement detected!");
   ...
   else
     {
      if(entry < freshPrice)
        {
         result = trade.SellStop(g_CurrentLot, entry, symbol, finalSL, finalTP, timeType, expiration, "EMA movement detected!");
         orderLabel = "SELL STOP";
        }
      else
        {
         result = trade.SellLimit(g_CurrentLot, entry, symbol, finalSL, finalTP, timeType, expiration, "EMA movement detected!");
