...
def parse_signal(text: str):
    ...
    # Range detect karne ke liye explicit dash chahiye (-, en-dash, em-dash) do numbers ke beech,
    # taaki galti se agli line ka SL/TP number range ka "doosra number" na ban jaye
    range_match = re.search(r'@\D{0,4}([\d.]+)\s*[-–—]\s*([\d.]+)', text)
    if range_match:
