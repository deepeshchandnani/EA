...
def parse_signal(text: str):
    ...
    if range_match:
        ...
        # Entry rule: BUY -> sabse BADA number, SELL -> sabse CHOTA number
        entry_deep = max(num1, num2) if direction == "BUY" else min(num1, num2)
        entry_near = min(num1, num2) if direction == "BUY" else max(num1, num2)
...
def save_close_signal():
    """Telegram par 'TP1 CLEAR' message aane par ye file banti hai - EA isse dekhte hi
    saari open positions turant close kar deta hai."""
    os.makedirs(SIGNAL_FOLDER, exist_ok=True)
...
async def handler(event):
    ...
    # "TP1 CLEAR" message - ye ek close-trigger hai, koi naya signal nahi.
    # Aate hi saari open positions close karne ka instruction EA ko bhej dete hain.
    if "TP1 CLEAR" in text.upper():
        print("[CLOSE TRIGGER] 'TP1 CLEAR' message mila - saari positions close karne ka signal bhej rahe hain.")
        if not LOG_ONLY:
            save_close_signal()
        else:
    ...
    # MT5 EA simple single-entry design use karta hai - backward-compatible
    # "entry"/"sl" fields bhi bhej dete hain
    mt5_signal = dict(signal)
    mt5_signal["entry"] = signal["entry_deep"]  # single entry - min(BUY)/max(SELL) logic
    mt5_signal["sl"] = signal["sl_raw"]          # asli signal ka SL - isi ko broker SL banayenge

    save_signal(mt5_signal, event.message.id)
